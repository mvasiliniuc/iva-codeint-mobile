...
from dataset_preprocessing_args import DatasetPreprocessingArguments
...
def has_no_keywords(example):
    """Check if a file has none of the keywords for Kotlin"""
    keywords = ["fun ", "val ", "var ", "if ", "else ", "while ", "for ", "return ", "class ", "data ", "struct ", "interface ", "when ", "catch "]
    lines = example["content"].splitlines()
    for line in lines:
        for keyword in keywords:
            if keyword in line.lower():
...
# Settings
parser = HfArgumentParser(DatasetPreprocessingArguments)
...
print(f"Time consumed: {time.time()-t_start:.2f}")
